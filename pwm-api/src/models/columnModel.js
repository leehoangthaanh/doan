...
const remove = async (columnId) => {
    ...
    const collection = GET_DB().collection(COLUMN_COLLECTION_NAME)
    const column = await collection.findOne({ _id: new ObjectId(columnId) })
    if (!column) return null

    const deleteResult = await collection.deleteOne({ _id: new ObjectId(columnId) })
